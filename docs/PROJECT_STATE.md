...
- 2026-04-19 — Phase 0 scaffolding: `package.json`, `tsconfig.json`, `.env.example`, `Dockerfile`, Fastify server skeleton, config + logger + errors, `LLMProvider` interface, docs bootstrapped, `git init` on `main`.

## What works right now

- `npm run typecheck` (once deps are installed)
- `GET /healthz` returns `{ ok: true }` once `.env` is populated

## What does not work yet

- No DB schema / migrations (Phase 1)
- No WATI webhook or outbound client (Phase 2)
- No matching pipeline (Phase 3)
- No consent flow (Phase 4)
- No tests (Phase 5)
