...
- 2026-05-05 — Cohort data + perf + quantitative reasoning. Three threads landed together. (1) **Real cohort data**: 255/312 records from the 7 cohort JSON dumps transformed into structured rows via `src/ingest/cohort.ts` — deterministic tag inference (no LLM at ingest), city normalization, synthetic phones in the `91-91xxxxxxxx` placeholder block (real numbers swap in by phone-keyed UPSERT later). DB wiped + reseeded (255 founders, 255 1536-dim embeddings, 244 with role tags, 73 with sector tags, 57 unique cities). (2) **Occurrence tracking** (migration `0003_founder_occurrence.sql`): `founders.times_shown` + `last_shown_at`. `recordShown` increments only on actual insert (so an `ON CONFLICT DO NOTHING` retry doesn't double count); the retriever adds `+ LEAST(times_shown, 50) * 0.005` to the cosine distance ORDER BY so globally over-shown founders drift down without becoming filterable hard-blocks. (3) **Perf**: rerank + intent extraction now route to `OPENAI_MODEL_RERANK` / `GEMINI_MODEL_RERANK` (default `gpt-4.1-mini` / `gemini-2.5-flash-lite`) — those knobs existed but weren't wired (json call hardcoded `OPENAI_MODEL_CHAT`); agent loop default flipped from `gemini-2.5-pro` → `gemini-2.5-flash`; trivial refinements ("next" / "skip" / "more") skip the intent LLM via `isTrivialRefinement` and use the existing `search_state` to compose `semantic_query` deterministically; per-process LRU cache (256 entries) for query embeddings on the hot path. (4) **Quantitative reasoning surfaced to user** (`rerank_v5`, `agent_v4`): rerank now emits `match_score` (0-100), per-axis `breakdown`, and `headline_evidence` (verbatim quotes); agent prompt requires the score + at least one piece of evidence on every card and forces axis-by-axis explanation when the founder pushes back. End-to-end smoke (`scripts/smoke-matching.ts`) returns scores like `Naren 100/100 (role=3 sector=3 stage=3 loc=3)` for healthtech-GTM-Bangalore against real cohort profiles. 67 tests green, typecheck clean.
- 2026-04-21 — Conversational quality, Batch B (B1 + B2 + B3). `rerank_v4` adds `intro_recommendation` ("warm" | "hold") and `hold_reason` alongside v3's fields; when the top card is a hold, the dispatcher flips to a "Holding off on this intro: …" body and a single `Force intro` button (`onForceIntro` records `action = 'forced'` on `candidates_shown` for override-rate measurement). `shouldShowTwo` + `formatTwoCardsText` pack two warm cards into one outbound when runner-up is within 60% of the top score — WATI's 3-button cap plus the one-outbound invariant make typed `1` / `2` / `Skip` the chosen reply model (deterministic parse in `classifyIntent`). `topic_switch` joins `VoiceIntent` for out-of-scope asks (investors, legal, cold emails) with a 0.7 confidence floor so a hallucinated switch can't nuke `search_state`. All three changes preserve the exactly-one-outbound-per-inbound contract the dispatch lock depends on. Shipped without a feature flag per request; rollback = PR revert. Prompts versioned (`rerank_v3` → `rerank_v4`). Spec at `docs/superpowers/specs/2026-04-21-conversational-quality-batch-b-design.md`. 81 tests green.
- 2026-04-21 — Conversational quality, Batch A. The bot was reading as a matcher wrapped in a menu: scripted greeting with "Examples: engineering, product, marketing" menu phrasing, "Closest fit right now" header + structured meta line, flat non-cohort rejection. Reshaped three things without touching the dispatcher contract: (1) `rerank_v3` now returns `bullets[]` and `drawback` alongside `rationale`, and `formatCardText` renders an operator-voice bullet card with an honest "Potential drawback:" line; (2) `voice_v3` greeting is now a two-beat "here's how I match / what's the one thing they must be great at" opener with an explicit ban on numbered lists and "separated by commas" phrasing; (3) `voice_v3` `non_cohort` is a warm single message pointing cohort-by-mistake users to the Build3 team and non-cohort users to build3.com. WATI dashboard runbook at `docs/WATI_CONFIG.md` documents the toggles (Welcome Message, keyword flows, default-assignee auto-reply) that were intercepting messages before our webhook fired. Prompts versioned per CLAUDE.md rule #3; rollback = single import flip. Spec at `docs/superpowers/specs/2026-04-21-conversational-quality-batch-a-design.md`. Batch B (qualified-no, 2-at-a-time, topic-switch intent) deferred.
